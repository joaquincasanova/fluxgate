...
for ac_index in range(1,ac_levels+1):
    for dc_index in range(1,dc_levels+1):
        ...
        Hm = (1e-12)/MU0
        Hearth = 50e-6/MU0
        Idc = 75e-3*dc_index/dc_levels#(A)
        ...
        Bz1 = B(Hz1,Hc,mu_i,Bs, sign)

        mur = dBdH(Hz1,Hc,mu_i,Bs, sign)/MU0
        dmur_dt = np.gradient(mur)/np.gradient(time)


        Phys=Pdiss(Bz1,Hz1,2*period,Acore*L)
        V1=-2*Nsense*PI*Acoretube*(Hearth)*(1-Dtube)/(1+Dtube*(mur-1))/(1+Dtube*(mur-1))*dmur_dt*MU0

        Hz2  = H(Idrive,Ndrive,L,Hm+Hearth)
        Bz2 = B(Hz2,Hc,mu_i,Bs, sign)

        mur = dBdH(Hz2,Hc,mu_i,Bs, sign)/MU0
        dmur_dt = np.gradient(mur)/np.gradient(time)
        V2=-2*Nsense*PI*Acoretube*(Hm+Hearth)*(1-Dtube)/(1+Dtube*(mur-1))/(1+Dtube*(mur-1))*dmur_dt*MU0
        ...
        Bz1_tube[loop_index,:]=Bz1
        Hz1_tube[loop_index,:]=Hz1
        Idrive_tube[loop_index,:]=Idrive
        Vtube1[loop_index,:] = V1
        Bz2_tube[loop_index,:]=Bz2
        Hz2_tube[loop_index,:]=Hz2
        Idrive_tube[loop_index,:]=Idrive
